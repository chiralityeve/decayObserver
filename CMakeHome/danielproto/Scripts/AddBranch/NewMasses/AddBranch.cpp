...
int main(){
    ...
    TFile fin("/afs/cern.ch/work/d/dberning/private/Pruned/Data/Data_merged_pruned.root", "READ");
    if(!fin.IsOpen()){
    ...
    TTree *oldlumi = (TTree*)fin.Get("GetIntegratedLuminosity/LumiTuple");
   
    TDirectory* Diroldtree = &fin;
    Diroldtree = Diroldtree -> GetDirectory("Bs2phimumuTuple");

    TDirectory* Diroldlumi = &fin;
    Diroldlumi = Diroldlumi -> GetDirectory("GetIntegratedLuminosity");
    ...
    TFile fout("/afs/cern.ch/work/d/dberning/private/Pruned/Data/Data_merged_pruned_newVars.root", "RECREATE");
