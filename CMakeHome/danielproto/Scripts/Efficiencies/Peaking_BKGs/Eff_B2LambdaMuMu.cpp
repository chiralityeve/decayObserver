...
int main() {
    ...
    TFile* file = TFile::Open("/afs/cern.ch/work/d/dberning/private/BDT_w_bdtana/Peaking_BKG_MCs/B2LambdaMuMu_MC_pruned_TMVAResponse.root");
    if (file == 0) {
    ...
    TFile* file_generator = TFile::Open("/afs/cern.ch/work/d/dberning/private/Pruned/Peaking_BKG_MCs/B2LambdaMuMu_MC_pruned.root");
    if (file_generator == 0) {
        // if we cannot open the file, print an error message and return immediatly
        // printf("Error: cannot open
        //       http://lcg-heppkg.web.cern.ch/lcg-heppkg/ROOT/eventdata.root!\n");
    ...
    TFile* file_BDT = TFile::Open("/afs/cern.ch/work/d/dberning/private/BDT_w_bdtana/Peaking_BKG_MCs/B2LambdaMuMu_MC_pruned_TMVAResponse.root");
    if (file_BDT == 0) {
        // if we cannot open the file, print an error message and return immediatly
        // printf("Error: cannot open
        //       http://lcg-heppkg.web.cern.ch/lcg-heppkg/ROOT/eventdata.root!\n");
    ...
    Value_w_Err generator_eff(0.3398, 0.0016);
    ...
    cout << "------ Preselectionefficiency (cut out resonances and cut on Bs-mass) -------" << endl << endl;
    ...
    //This decay----------------------------------
    //BR(this)
    Value_w_Err Bthis1(1.08 * pow(10, -6), 0.28 * pow(10, -6));     //Lambda_b -> Lambda Mu Mu  NOTE: Branching Ratio nach Lambda(1520)MuMu nicht bekannt 
    Value_w_Err Bthis2(0.45 / 2, 0.01 / 2);                          //Lambda(1520) -> NK und davon die Hälfte (wollen nur pK)
    ...
    Value_w_Err fs_fufd(0.132, 0.010);           //Value of fs/(fu+fd)
    Value_w_Err flambdab_fufd(0.305, 0.022);     //Value of flambdab/(fu+fd)
    Value_w_Err flambdab_fs = flambdab_fufd / fs_fufd;   //Value of flambdab/fs

    cout << "flambdab/fs: "; flambdab_fs.Print(); cout << endl << endl;

    //Result
    Value_w_Err Npeaking_punzi = Nres / (Bres * Res_punzi_eff) * flambdab_fs * Bthis * total_punzi_eff;
