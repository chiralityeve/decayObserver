...
int main() {
    ...
    //Daten (ALLE)
    TFile* fileData = TFile::Open("/afs/cern.ch/work/d/dberning/private/RawData/Fulldata.root");
    ...
    TFile* tmpfile = new TFile("./tmptree.root", "RECREATE");
    TTree* treecut = treeData -> CopyTree(allcuts.c_str(), "", 400000);

    TFile* tmpfile2 = new TFile("./tmptree2.root", "RECREATE");
    TTree* treecut_hard = treeData -> CopyTree(allcuts_hard.c_str(), "", 400000);
    ...
    TH1D* hist_f2 = new TH1D("combimass", "Kombinierte Masse von K und p", 200, 1400, 4000);
    TH1D* hist_B = new TH1D("combimass1", "Kombinierte Masse des B-Mesons", 200, 4000, 5600);

    TH1D* hist_f2_cut = new TH1D("combimass2", "Kombinierte Masse von K und p (cut kaon_PIDK > -3 applied)", 200, 1400, 4000);
    TH1D* hist_B_cut = new TH1D("combimass3", "", 200, 4000, 5600);
    
    TH1D* hist_f2_hardcut = new TH1D("combimass4", "Kombinierte Masse von K und p (kaon_PIDK and cut on f2-mass applied)", 200, 1400, 4000);
    ...
    hist_f2_cut -> Draw();
    c.SaveAs("/afs/cern.ch/user/d/dberning/private/Commongit/decayObserver/CMakeHome/plots/Protonhypothese/Nur_Kaon_DLLK.png");

    
    hist_f2_hardcut -> Draw();
    c.SaveAs("/afs/cern.ch/user/d/dberning/private/Commongit/decayObserver/CMakeHome/plots/Protonhypothese/Komplette_Vorselektion.png");
