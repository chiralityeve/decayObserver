...
// main GUI
void TMVAGui( const char* fName = "Bs2mumuf2_nonresonant_FINAL.root" ) 
{   
    ...
    //Standardpath
    std::string path = "/afs/cern.ch/work/d/dberning/private/BDT/Trainingoutput/";
    std::string file = path + fName;
    ...
    cout << "--- Launch TMVA GUI to view input file: " << file.c_str() << endl;

    // init
    TMVAGui_inactiveButtons.clear();

    // check if file exist
    TFile* file_ex = TFile::Open( file.c_str() );
    if (!file_ex) {
        cout << "==> Abort TMVAGui, please verify filename" << endl;
        return;
    }
    // find all references   
    TMVAGui_keyContent = (TList*)file_ex->GetListOfKeys()->Clone();
    ...
    while ((str = (TObjString*)it())) {
        ...
        ActionButton( cbar, 
                ...
                Form( ".x variables.C(\"%s\",\"%s\",\"%s\")", file.c_str(), str->GetString().Data(), title.Data() ),
    ...
    while ((str = (TObjString*)it())) {
        ...
        ActionButton( cbar, 
                Form( "(%i%c) %s", ic, ch++, title.Data() ),
                Form( ".x CorrGui.C(\"%s\",\"%s\",\"%s\")", file.c_str(), str->GetString().Data(), title.Data() ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x correlations.C(\"%s\")", file.c_str() ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x efficiencies.C(\"%s\")", file.c_str() ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x efficiencies.C(\"%s\",%d)", file.c_str(), 3 ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x paracoor.C(\"%s\")", file.c_str() ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x likelihoodrefs.C(\"%s\")", file.c_str() ), 
    ...
    TString call = Form( ".x network.C+g(\"%s\")", file.c_str() );
    ActionButton( cbar,  
            title,
            call, 
    ...
    ActionButton( cbar,  
            title,
            Form( ".x annconvergencetest.C(\"%s\")", file.c_str() ), 
    ...
    ActionButton( cbar,  
            title,
            Form( ".x BDTControlPlots.C+(\"%s\")", file.c_str() ),
    ...
    //                  Form( ".x rulevis.C(\"%s\",0)", file.c_str() ),
    ...
    ActionButton( cbar,  
            title,
            Form( ".x BoostControlPlots.C(\"%s\")", file.c_str() ),
