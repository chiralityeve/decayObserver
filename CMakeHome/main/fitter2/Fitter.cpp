...
Fitter& Fitter::operator =(const Fitter& other)
{
    ...
    integralranges = other.integralranges;
...
//Set Ranges for which integrals shall be calculated
void Fitter::CreateIntegralrange(double lowlimit, double uplimit) {
    Interval integralrange(lowlimit, uplimit);

    integralranges.push_back(integralrange);
}


//Calculate Integral of the PDFs which are (currently) already added to the Fitter
void Fitter::CalculateIntegrals() 
{
    if(integralranges.empty()) return;
    ...
    for( auto& integralrange : integralranges) {
        i += 1;
        rangename = "Integralrange" + std::to_string(i);
        pFitVar -> setRange(rangename.c_str() , integralrange.minimum, integralrange.maximum); 

        RooAbsReal* intOfTotal = pTotPdf -> createIntegral(*pFitVar, RooFit::NormSet(*pFitVar), RooFit::Range(rangename.c_str()));
        RooAbsReal* intOfSignal = pSigPdf -> createIntegral(*pFitVar, RooFit::NormSet(*pFitVar), RooFit::Range(rangename.c_str()));
        RooAbsReal* intOfBackground = pBkgPdf -> createIntegral(*pFitVar, RooFit::NormSet(*pFitVar), RooFit::Range(rangename.c_str()));
        ...
        std::cout << "\tIntegral(" << integralrange.minimum << ", " << integralrange.maximum << "):\t "; integral.Print(); std::cout << std::endl;

        std::cout << std::endl << std::endl << "------------------------ Integral of SignalPdfs --------------------------------" << std::endl;
        std::cout << "\tTotal yield:\t\t "; totalyieldsignal.Print(); std::cout << std::endl;
        std::cout << "\tIntegral(" << integralrange.minimum << ", " << integralrange.maximum << "):\t "; integral_signal.Print(); std::cout << std::endl;  
        ...
        std::cout << "\tIntegral(" << integralrange.minimum << ", " << integralrange.maximum << "):\t "; integral_background.Print(); std::cout << std::endl << std::endl << std::endl;
    }
}
